...
execute if score BlueCartCount Process < BlueCartCount Variable as @e[type=marker,tag=blue_tnt_spawner] at @s unless entity @e[type=command_block_minecart,distance=..1] run summon command_block_minecart ~ ~ ~ {Invulnerable:true,Tags:["TNTcart","blue"],CustomDisplayTile:1,DisplayState:{Name:"minecraft:tnt"},DisplayOffset:6,Command: "Yes I am secretly a commandblock minecart ;)"}

...
execute if entity @s[team=spectator] unless score Spectators gold matches 1.. run tellraw @s ["",{"text":"Not enough funds, ","color":"gray"},{"text":"1 gold","color":"gold"},{"text":" required!","color":"gray"}]
execute if entity @s[team=spectator] if score Spectators gold matches 1.. run give @s vine{display:{Name:'[{"text":"Elevator","italic":false}]',Lore:['[{"text":"Works on Minecarts and players alike"}]']}} 16
execute if entity @s[team=spectator] if score Spectators gold matches 1.. run scoreboard players remove Spectators gold 1
